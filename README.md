...
```bash
git clone https://github.com/dawith-ai/claude-terminal-auto.git
cd claude-terminal-auto
./install.sh
```

`install.sh`가 plist의 경로를 이 폴더에 맞춰 `~/Library/LaunchAgents`에 설치하고 launchd에 등록합니다(재부팅에도 유지). 상태 확인:
