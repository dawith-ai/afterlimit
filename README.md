...
- **tmux-resume** (`scripts/tmux_resume_watcher.py`): reads every pane with `tmux capture-pane`; when the limit menu (`What do you want to do? / 1. Stop and wait for limit to reset / 2. Upgrade your plan`) is **active** at the bottom, it uses `tmux send-keys` to press **`1` → `Enter`**, confirming option 1 → Claude resumes your work at reset. (⚠️ It never sends `Esc`, which would cancel the menu.) **False-positive guards**: fires only when all three menu phrases are present and the normal input bar (`bypass permissions`) / generating state (`esc to interrupt`) is not; 5-minute cooldown per pane.
