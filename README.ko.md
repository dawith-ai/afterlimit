...
- **tmux-resume** (`scripts/tmux_resume_watcher.py`): `tmux capture-pane`로 모든 pane을 읽어 한도 결정 메뉴(`What do you want to do? / 1. Stop and wait for limit to reset / 2. Upgrade your plan`)가 **활성 상태**로 하단에 떠 있으면, `tmux send-keys`로 **`1` → `Enter`**를 눌러 옵션 1을 확정합니다 → Claude가 리셋 시각에 작업을 자동으로 이어감. (⚠️ Esc는 메뉴를 취소하므로 절대 안 씀.) **오탐 차단**: 메뉴 세 문구가 다 있고 일반 입력바(`bypass permissions`)·생성중(`esc to interrupt`)이 아닐 때만 발사, pane당 5분 쿨다운.
